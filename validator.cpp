#include "validator.h"
#include <QRegularExpression>
#include <QJsonValue>

validator::validator() {}

// Проверка на допустимые символы (алфавиты, цифры, подчёркивание, дефис)
static bool isValidName(const QString& name) {
    static QRegularExpression re("^[A-Za-z0-9_А-Яа-яёЁ\\-]+$");
    return re.match(name).hasMatch();
}

bool validateInput(const QJsonObject& json, QSet<Error>& errors, QVector<Class>& parsedClasses) {
    // Проверка корневых ключей на лишние поля
    for (const QString& key : json.keys()) {
        if (key != "classes") {
            errors.insert(Error(ErrorType::extraField, key));
        }
    }

    if (!json.contains("classes") || !json.value("classes").isArray()) {
        errors.insert(Error(ErrorType::emptyClassesArray));
        return false;
    }

    QJsonArray classesArray = json.value("classes").toArray();
    if (classesArray.isEmpty()) {
        errors.insert(Error(ErrorType::emptyClassesArray));
        return false;
    }
    if (classesArray.size() > 100) {
        errors.insert(Error(ErrorType::tooManyClasses, "", "", 0, classesArray.size()));
    }

    for (int i = 0; i < classesArray.size(); ++i) {
        if (!classesArray.at(i).isObject()) continue;
        QJsonObject classObj = classesArray.at(i).toObject();

        for (const QString& key : classObj.keys()) {
            if (key != "class_name" && key != "properties") {
                errors.insert(Error(ErrorType::extraField, key));
            }
        }

        if (!classObj.contains("class_name")) {
            errors.insert(Error(ErrorType::missingClassName, "", "", 0, i + 1));
            continue;
        }

        QJsonValue nameVal = classObj.value("class_name");
        if (!nameVal.isString()) {
            errors.insert(Error(ErrorType::invalidCharacters, QString("класса №%1").arg(i + 1)));
            continue;
        }

        QString className = nameVal.toString();
        if (className.isEmpty() || className.length() > 255) {
            errors.insert(Error(ErrorType::invalidClassNameLength, className, "", 0, className.length()));
        }
        if (!isValidName(className)) {
            QString badChar;
            for (QChar ch : className) {
                if (!isValidName(QString(ch))) {
                    badChar = QString(ch);
                    break;
                }
            }
            errors.insert(Error(ErrorType::invalidCharacters, className, badChar));
        }

        if (!classObj.contains("properties") || !classObj.value("properties").isArray()) {
            errors.insert(Error(ErrorType::missingProperties, className));
            continue;
        }

        QJsonArray propArray = classObj.value("properties").toArray();
        if (propArray.isEmpty()) {
            errors.insert(Error(ErrorType::emptyProperties, className));
        }
        if (propArray.size() > 100) {
            errors.insert(Error(ErrorType::tooManyProperties, className, "", 0, propArray.size()));
        }

        Class cls;
        cls.name = className;

        for (int j = 0; j < propArray.size(); ++j) {
            if (!propArray.at(j).isObject()) continue;
            QJsonObject propObj = propArray.at(j).toObject();

            for (const QString& key : propObj.keys()) {
                if (key != "name" && key != "value_count" && key != "expected_value") {
                    errors.insert(Error(ErrorType::extraField, key));
                }
            }

            if (!propObj.contains("name")) {
                errors.insert(Error(ErrorType::missingPropertyName, className, "", 0, j + 1));
                continue;
            }

            QJsonValue propNameVal = propObj.value("name");
            if (!propNameVal.isString()) {
                errors.insert(Error(ErrorType::invalidCharacters, QString("свойства №%1 в классе %2").arg(j + 1).arg(className)));
                continue;
            }

            QString propName = propNameVal.toString();
            if (propName.isEmpty() || propName.length() > 255) {
                errors.insert(Error(ErrorType::invalidPropertyNameLength, className, "", 0, propName.length()));
            }
            if (!isValidName(propName)) {
                QString badChar;
                for (QChar ch : propName) {
                    if (!isValidName(QString(ch))) {
                        badChar = QString(ch);
                        break;
                    }
                }
                errors.insert(Error(ErrorType::invalidCharacters, propName, badChar));
            }

            Property prop;
            prop.name = propName;
            prop.ruleType = PropertyRuleType::HasProperty;

            if (propObj.contains("value_count") && propObj.contains("expected_value")) {
                errors.insert(Error(ErrorType::ambiguousRule, propName));
            }

            if (propObj.contains("value_count")) {
                QJsonValue vcVal = propObj.value("value_count");
                if (!vcVal.isArray()) {
                    errors.insert(Error(ErrorType::invalidValueCountType, propName));
                } else {
                    QJsonArray vcArray = vcVal.toArray();
                    if (vcArray.size() != 1) {
                        errors.insert(Error(ErrorType::invalidValueCountType, propName));
                    } else if (!vcArray.at(0).isDouble()) {
                        errors.insert(Error(ErrorType::invalidValueCountType, propName));
                    } else {
                        double dVal = vcArray.at(0).toDouble();
                        int val = vcArray.at(0).toInt();
                        if (dVal != val || val < 1 || val > 1000) {
                            errors.insert(Error(ErrorType::invalidValueRange, propName, "", val));
                        } else {
                            prop.valueCount.append(val);
                            prop.ruleType = PropertyRuleType::HasPropertyWithCount;
                        }
                    }
                }
            }

            if (propObj.contains("expected_value")) {
                QJsonValue evVal = propObj.value("expected_value");
                if (!evVal.isArray()) {
                    errors.insert(Error(ErrorType::invalidValueType, propName));
                } else {
                    QJsonArray evArray = evVal.toArray();
                    if (evArray.isEmpty()) {
                        errors.insert(Error(ErrorType::emptyExpectedValue, propName));
                    } else if (evArray.size() > 100) {
                        errors.insert(Error(ErrorType::tooManyExpectedValues, propName, "", 0, evArray.size()));
                    } else {
                        bool validItems = true;
                        for (int k = 0; k < evArray.size(); ++k) {
                            if (!evArray.at(k).isDouble()) {
                                errors.insert(Error(ErrorType::invalidValueType, propName));
                                validItems = false;
                                break;
                            }
                            double dVal = evArray.at(k).toDouble();
                            int val = evArray.at(k).toInt();
                            if (dVal != val || val < 1 || val > 1000) {
                                errors.insert(Error(ErrorType::invalidValueRange, propName, "", val));
                                validItems = false;
                                break;
                            }
                            prop.expectedValues.append(val);
                        }
                        if (validItems) {
                            if (evArray.size() == 1) {
                                prop.ruleType = PropertyRuleType::HasPropertyWithValue;
                            } else {
                                prop.ruleType = PropertyRuleType::HasPropertyWithValues;
                            }
                        }
                    }
                }
            }

            cls.properties.append(prop);
        }
        parsedClasses.append(cls);
    }

    return errors.isEmpty();
}
